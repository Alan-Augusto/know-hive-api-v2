...
export class QuestionResponseService {
  ...
  async create(createQuestionResponseDto: CreateQuestionResponseDto) {
    const { question_id, user_id, alternative_id, collection_id, response_time } = createQuestionResponseDto;
    ...
    // Verificar se a alternativa existe e pertence à questão
    ...
    if (!alternative) {
      throw new BadRequestException(`Alternativa com ID ${alternative_id} não encontrada ou não pertence à questão ${question_id}`);
    }
    ...
    return {
      ...questionResponse,
      is_correct: questionResponse.alternative.is_correct,
    };
  }
